...
%% I did it so that  I could arrange it in the order with sub title
...
the modifiers. As mentioned earlier in the abstract, we decided to implement 
the project progressively where we first focused on propositional fallacies. For the kind of fallacies we decided to detect first, we needed the logical form of the expression. So we started implementing sentence parsing and turning the sentence into propositional logic form. Following schematic logic was used to convert the text to logical form.
\item Set the boundaries so that separate knowledge can be extracted. Replace
...
(Stemming separates the sentences with \textquotedbl{}.\textquotedbl{}
